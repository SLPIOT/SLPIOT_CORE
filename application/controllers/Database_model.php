...
defined('BASEPATH') OR exit('No direct script access allowed');
   class database_model extends CI_Model {
      ...
      function getLatestRecords($stationID){
        $code = " stationID=\"". $stationID."\" ORDER BY Record_time DESC  Limit 0,10";
	...
	$rate = 1;
	$groupby ="floor(hour(Record_time)/".$rate.")";
		
	$code = " stationID=\"". $stationID."\" Group by ".$groupby." ORDER BY 'Record_time' DESC Limit 10";
		
		
        //$code = " stationID='".$stationID."'  Group by ID Having 'ID'%10 =0 ORDER BY 'Record_time' DESC Limit 10";
        
	return $this->database_model->getAllDataWithParams ("data_stream","ROUND(AVG(".$name_."),4)",$code); 
